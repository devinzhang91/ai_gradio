...
def call_bing_search(keyword, start=1, num=10):
    ...
    headers = { 'Ocp-Apim-Subscription-Key':  BingSearchConf.API_KEY }

    # logger.debug(f"headers={headers}, params={params}")
    
    # Call the API
    try:
        response = requests.get(BingSearchConf.API_URL, headers=headers, params=params)
