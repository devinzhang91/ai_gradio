...
def call_google_search(keyword, start=1, num=10):
    ...
    payload = {'cx': GoogleSearchConf.API_CX, 
               'key': GoogleSearchConf.API_KEY,
               'start': str(start),
               'num': str(num),
               'q': keyword}
    logger.debug(f"payload={payload}")
    response = requests.get(GoogleSearchConf.API_URL, params=payload)
    # print(response.json())
...
#     print("Google Search")
#     # call_google_search("python")
#     json = call_google_search("python", num=10)
#     parse_google_search_result(json)