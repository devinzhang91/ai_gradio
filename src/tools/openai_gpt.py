...
def call_openai_token(prompt:str,
                      engine: str = OpenAiConf.PLAYGROUND_MODELS_LIST[0],
                      max_tokens: int = 500):
    prompt = query_prompt_template.format(chat_history="", question=prompt)
    try:
        kwargs = dict(
            prompt=prompt,
            engine=OpenAiConf.PLAYGROUND_MODELS_LIST[0],
            temperature=0.0, 
            max_tokens=max_tokens, 
            n=1, )
...
def call_openai_chat_token(messages,
    ...
    prompt = query_prompt_template.format(chat_history=get_chat_history_as_text(messages, False), question=messages[-1]["content"])
...
def call_openai_chat_with_search(messages: list,
    ...
    for result in search_results:
        ...
        if result == search_results[max_search_results-1]:
            break
    prompt = prompt_prefix.format(injected_prompt="", sources=result_text, chat_history=get_chat_history_as_text(messages), follow_up_questions_prompt="")
    try:
        kwargs = dict(
            prompt=prompt,
            # engine=engine,
...
def call_openai_chat_with_search2(messages: list,
                                search_results,
                                engine: str = OpenAiConf.CHATBOT_MODELS_LIST[0],
                                temperature: float = 0.2,
                                top_p: float = 0.80,
